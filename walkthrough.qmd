...
Several methods within the Bayesian inference procedures can be utilized to *estimate* the posterior distribution of the parameter, and most of these fall into the category of *Markov Chain Monte Carlo methods (MCMC)*. *MCMC* are methods to indirectly simulate random observations from probability distributions using stochastic processes [@Everitt_et_al_2010] [^2]. However, when the parameters of interest are not large in number, a useful *pedagogical* method to produce the posterior distribution is the *grid approximation* method. Through this method, an excellent approximation of the parameter's posterior distribution can be achieved by considering a finite candidate list of parameter values. This method is used in @sec-howitworks to illustrate how the Bayesian inference works [^3]. 
...
RQ1_WAIC = compare( func=WAIC, # <2>
                    model01, model04,
                    model07, model10 )
RQ1_WAIC = cbind( DIC=with(RQ1_WAIC, WAIC-2*pWAIC), # <3>
                  RQ1_WAIC )

```
1. seed for replication
2. comparison of selected models with WAIC
3. DIC calculation
...
RQ1_PSIS = compare( func=PSIS, # <2>
                    model01, model04,
                    model07, model10 )
RQ1_PSIS = cbind( DIC=with(RQ1_PSIS, PSIS-2*pPSIS), # <3>
                  RQ1_PSIS )

```
1. seed for replication
2. comparison of selected models with PSIS
3. DIC calculation
...
```{r}
#| label: tbl-rq1-waic
#| tbl-cap: "WAIC comparison for selected models"
...
```{r}
#| label: tbl-rq1-psis
#| tbl-cap: "PSIS comparison for selected models"
...
SI = contrast_SI(d=data_H, stanfit_obj=model10, # <1>
...
for(i in idx_comp){
  dens( SI$SI_raw[[i]], xlim=c(-2.5,2.5), # <1>
...
RQ3_PSIS = compare( func=PSIS, # <2>
                    model01, model02, model03,
                    model04, model05, model06,
                    model07, model08, model09,
                    model10, model11, model12 )
RQ3_PSIS = cbind( DIC=with(RQ3_PSIS, PSIS-2*pPSIS), # <3>
                  RQ3_PSIS )
...
```{r}
#| label: tbl-rq3-waic
#| tbl-cap: "WAIC comparison for all models"
...
```{r}
#| label: tbl-rq3-psis
#| tbl-cap: "PSIS comparison for all models"
...
#| tbl-cap: "Model 10, parameter estimates and 95% highest probability density intervals (HPDI)"
...
model_par = par_recovery(stanfit_obj=model11, # <2>
                         p=0.95, est_par=par_int)
contrs = contrast_intel(stanfit_obj=model11, p=0.95, # <3>
                        rope=c(-0.05,0.05))
...
1. parameters of interest
2. user defined function: recovers the hearing status and chronological age parameter estimates for selected model
3. user defined function: extracts contrast of interest from selected model

```{r}
#| label: tbl-parameter-model11
#| tbl-cap: "Model 11, parameter estimates and 95% highest probability density intervals (HPDI)"
...
par_int = c('a','aHS[1]','aHS[2]','bAm','bAmHS[1]','bAmHS[2]') # <1>
model_par = par_recovery(stanfit_obj=model12, # <2>
                         p=0.95, est_par=par_int)
contrs = contrast_intel(stanfit_obj=model12, p=0.95, # <3>
                        rope=c(-0.05,0.05))
...
#| tbl-cap: "Model 12, parameter estimates and 95% highest probability density intervals (HPDI)"
...
**Availability of data and materials:** The data is delivered upon request, while the user-defined functions are available in the `code` folder from this walk-through [repository](https://github.com/jriveraespejo/betaprop.walkthrough.github.io)

**Code availability:** The code is available this  [walk-through](https://jriveraespejo.github.io/betaprop.walkthrough.github.io/).
